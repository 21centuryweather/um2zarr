...
def fix_latlon_coord(cube, grid_type, dlat, dlon):
    ...
    if len(lat.points) == 180:
        ...
        np.allclose(-90.0 + 0.5 * dlat, lat.points[0]) and grid_type == "ND"
    ):
        lat.var_name = "lat_v"
    else:
        lat.var_name = "lat"
...
def cubewrite(cube, sman, compression, use64bit, verbose):
    ...
    if cube.data.dtype.kind == "f":
        ...
        fill_value = default_fillvals[
            "%s%1d" % (cube.data.dtype.kind, cube.data.dtype.itemsize)
        ]
    ...
    if c.shape == heaviside.shape:
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            c.data = np.ma.masked_array(
                c.data / heaviside.data, heaviside.data <= hcrit
            ).astype(np.float32)
    else:
        ...
        if set(c_p.points).issubset(h_p.points):
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                c.data = np.ma.masked_array(
                    c.data / h_tmp.data, h_tmp.data <= hcrit
                ).astype(np.float32)
        else:
            raise Exception(
...
def process(infile, outfile, args):
    ...
    if ff.fixed_length_header.grid_staggering == 6:
        ...
        raise Exception(
            "Unable to determine grid staggering from header %d"
        ...
        for c in cubes:
            ...
            if c.standard_name and umvar.standard_name:
                if c.standard_name != umvar.standard_name:
                    if args.verbose:
                        sys.stderr.write(
                            "Standard name mismatch %d %d %s %s\n"
                            % (
                                stashcode.section,
                                stashcode.item,
                                c.standard_name,
            ...
            if (
                not args.nomask
                and stashcode.section == 30
                and (201 <= stashcode.item <= 288 or 302 <= stashcode.item <= 303)
            ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="specify kind of netCDF format for output file: 1 classic, 2 64-bit offset, 3 netCDF-4, 4 netCDF-4 classic model. Default 3",
        choices=[1, 2, 3, 4],
    )
    parser.add_argument(
        "-c",
